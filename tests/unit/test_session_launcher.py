...
class TestSessionEnvApiPortExport:
    """``control_api_port: 0`` must never reach an agent as a port.

    0 is the "bind any free port" sentinel. Exporting it verbatim gave
    agents an unreachable ``ISSUE_ORCHESTRATOR_API_PORT=0`` that looked
    configured, shadowed the live port the review exchange injects, and
    made every agent callback undeliverable (#6913).
    """

    @staticmethod
    def _env_for(port: int, tmp_path: Path) -> str:
        ...
        launcher = object.__new__(SessionLauncher)
        launcher.config = cast(object, _Cfg(control_api_port=port))
        run_assets = MagicMock()
        run_assets.run_dir = tmp_path / "run"
        return SessionLauncher._build_session_env(
            launcher,
            completion_path="c.json",
            session_id="coding-1",
    ...
    def test_sentinel_port_is_not_exported(self, tmp_path: Path) -> None:
        assert "API_PORT" not in self._env_for(0, tmp_path)

    def test_real_port_is_exported(self, tmp_path: Path) -> None:
        env = self._env_for(59957, tmp_path)
        assert "ISSUE_ORCHESTRATOR_API_PORT='59957'" in env
