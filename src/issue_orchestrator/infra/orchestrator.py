...
from ..control.github_workflow import (
    GitHubWorkflow,
    launch_issue_by_number as _gw_launch_issue_by_number,
    get_issue_machine as _gw_get_issue_machine,
)
from ..control.worktree_manager import (
    get_worktree_path,
    get_session_name,
...
class Orchestrator:
    ...
    _inflight_stable_ids: dict[str, float] = field(
        default_factory=dict, init=False
    )  # stable_id -> expires_at (monotonic)
    ...
    _external_close_lock: threading.Lock = field(
    ...
    def _cleanup_manager(self) -> CleanupManager:
        return CleanupManager(
            ...
            lambda name: _kill_session(
                name, self.deps.session_manager, self.deps.events
            ),
            lambda name: _session_exists(
                name, self.deps.session_manager, self.deps.events
            ),
            lambda issue_number, agent_config: get_worktree_path(
                self.config, issue_number, agent_config
            ),
    ...
    def _session_launcher(self) -> SessionLauncher:
        return self.deps.session_launcher_factory(
            board_snapshot_provider=StateBoardSnapshotProvider(
                self.deps.board_snapshot_builder, lambda: self.state
            ),
            session_exists_fn=lambda name: _session_exists(
                name, self.deps.session_manager, self.deps.events
            ),
    ...
    def _plan_applier(self) -> OrchestratorSupport:
        return OrchestratorSupport(
            ...
            get_review_machine=self._get_review_machine,
            kill_session=lambda name: _kill_session(
                name, self.deps.session_manager, self.deps.events
            ),
    ...
    def session_launcher_callback(
        self, session_type: "SessionType", number: int
    ) -> Optional[Session]:
    ...
    def _launch_issue_by_number(self, n: int) -> Optional[Session]:
        return _gw_launch_issue_by_number(
            n,
            self.state.cached_queue_issues,
            self.launch_session,
            lambda: setattr(
                self.state, "issues_started_count", self.state.issues_started_count + 1
            ),
        )

    def _launch_review_by_number(self, n: int) -> Optional[Session]:
        return _ch_launch_review_by_number(
            n, self.state.pending_reviews, self.launch_review_session
        )
    ...
    def _launch_rework_by_number(self, n: int) -> Optional[Session]:
        return _ch_launch_rework_by_number(
            n, self.state.pending_reworks, self.launch_rework_session
        )

    def launch_validation_retry_by_number(self, n: int) -> Optional[Session]:
        retry = next(
            (r for r in self.state.pending_validation_retries if r.issue_number == n),
            None,
        )
        if retry is None:
            return None
        return _launch_validation_retry_session(
            retry,
            self.state,
    ...
    def _launch_tech_lead_by_number(self, n: int) -> Optional[Session]:
        return _ch_launch_tech_lead_by_number(
            n, self.state.pending_tech_lead_reviews, self.launch_tech_lead_session
        )
    ...
    def _get_session_machine(
        self, name: str, n: int, timeout: int
    ) -> Optional[SessionStateMachine]:
        return _sl_get_session_machine(
            name, n, timeout, self.deps.state_machine_manager
        )
    ...
    def _create_session(
        self, name: str, cmd: str, wd: Path, title: str | None = None
    ) -> bool:
        return _create_session(
            name, cmd, wd, title, self.deps.session_manager, self.deps.events
        )
    ...
    def _startup_manager(self) -> StartupManager:
        return StartupManager(
            ...
            lambda: extract_issue_branches(
                self.deps.working_copy, self.config.repo_root
            ),
    ...
    def launch_session(
        self, issue: Issue, *, tech_lead_scope: "TechLeadLaunchScope | None" = None
    ) -> Optional[Session]:
    ...
    def _check_e2e_completion(self) -> None:
        ...
        for orch_id in finished:
            ...
            event_name = (
                EventName.E2E_COMPLETED if status == "passed" else EventName.E2E_FAILED
            )
            self.deps.events.publish(
                TraceEvent(
                    event_name,
    ...
    def _maybe_run_sqlite_backups(self) -> None:
        ...
        interval_seconds = max(
            60, self.config.sqlite_backup.check_interval_minutes * 60
        )
    ...
    def _check_health(self) -> HealthDecision:
        return cast(
            HealthDecision, _check_health(self.deps.health_gate, self.state.paused)
        )
    ...
    def _reconcile_running_sessions(self) -> None:
        ...
        should_scan, trigger = self._orphan_reconcile_trigger(
            scan_started, active_count
        )
        if not should_scan:
        ...
        if not untracked:
            _recover_unresolved_work(
                self.state, self.deps.pending_work_claims, self.deps.claim_quarantine
            )
            return
    ...
    def _orphan_reconcile_trigger(
        self, now: float, active_count: int
    ) -> tuple[bool, str]:
    ...
    def _check_lease_renewals(self) -> None:
        ...
        for session in lost_sessions:
            ...
            self.state.active_sessions = [
                s
                for s in self.state.active_sessions
    ...
    def _handle_loop_error(self, error: Exception) -> None:
        ...
        logger.exception(
            "[LOOP] Error in iteration %d: %s", self._loop_iteration, error
        )
        self.deps.events.publish(
            TraceEvent(
    ...
    async def run_loop(self) -> None:
        ...
        self._last_network_sync, self._last_ui_update, self._loop_iteration = (
            0.0,
            time.time(),
            0,
        )
    ...
    def _fetch_all_issues(
        self, required_stable_ids: set[str] | None = None
    ) -> list[Issue]:
        return self._github_workflow.fetch_all_issues(
    ...
    def _update_dependency_problems(
        self, dep_blocked: list[tuple["Issue", str]]
    ) -> None:
    ...
    def launch_retrospective_review_session(
        ...
        return _launch_retrospective_review_session(
            review,
            self.state,
            self._session_launcher,
            self.deps.session_restorer,
    ...
    def launch_queued_tech_lead_session(
        ...
        return _launch_tech_lead_session(
            tech_lead,
            self.state,
            self.config,
            self._session_launcher,
    ...
    def process_deferred_cleanups(self) -> None:
        self.state.pending_cleanups = self._github_workflow.process_deferred_cleanups(
            self.state.pending_cleanups, self._cleanup_manager
        )
    ...
    def reconcile_orphaned_pr_labels(self) -> int:
        return self._github_workflow.reconcile_orphaned_pr_labels(
            ORCHESTRATOR_PR_MARKER
        )

    def launch_rework_session(self, rework: PendingRework) -> Optional[Session]:
        return _launch_rework_session(
            rework,
            self.state,
            self._session_launcher,
            self.deps.session_restorer,
