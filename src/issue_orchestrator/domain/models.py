...
def _session_monotonic_now() -> float:
    """Indirection keeps process-local session clocks deterministic in tests."""
...
class Session:
    ...
    watchdog_started_at_monotonic: float = field(
        default_factory=_session_monotonic_now
    )
    ...
    def __post_init__(self) -> None:
        ...
        if (
            type(self.watchdog_started_at_monotonic) is not float
            or not math.isfinite(self.watchdog_started_at_monotonic)
        ):
            raise ValueError(
                "Session.watchdog_started_at_monotonic must be a finite float"
            )
    ...
    def runtime_minutes(self) -> int:
        """How long this process-local watchdog has run, using monotonic time."""
        return int(self.runtime_seconds / 60.0)

    @property
    def runtime_seconds(self) -> float:
        """Monotonic elapsed time; wall timestamps remain diagnostic only."""
        elapsed = time.monotonic() - self.watchdog_started_at_monotonic
        if elapsed < 0:
            raise RuntimeError("session watchdog monotonic clock moved backward")
        return elapsed
